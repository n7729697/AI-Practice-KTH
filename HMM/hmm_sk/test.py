...
def baum_welch(O, A, B, pi, max_iter):
    ...
    log_likelihood = 1
    iter_cnt = 0
    while iter_cnt < max_iter and log_likelihood > old_log_likelihood:
        iter_cnt+=1
        if iter_cnt != 1:
...
class PlayerControllerHMM(PlayerControllerHMMAbstract):
    ...
    def reveal(self, correct, fish_id, true_type):
        ...
        if not correct:
            print(self.obs_now)
            A, B, PI = baum_welch(self.obs_now, self.models[true_type].A, self.models[true_type].B, self.models[true_type].PI, 5)
